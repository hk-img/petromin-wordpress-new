    ...
    <div class="view flex flex-col md:gap-y-12 gap-y-8" id="faqAccordion">
        ...
        <div class="w-full relative flex flex-col md:gap-y-16 gap-y-12 mt-6">
            <?php foreach ($processed_faq_categories as $category) : ?>
                <?php if (!empty($category['title']) && !empty($category['items'])): ?>
                <div class="flex flex-col gap-6 md:gap-y-5 w-full">
                    ...
                    <?php 
                    // Split category items into first and second columns (equal distribution)
                    $category_items = $category['items'];
                    $total_items = count($category_items);
                    $first_column_count = ceil($total_items / 2);
                    $first_column_items = array_slice($category_items, 0, $first_column_count);
                    $second_column_items = array_slice($category_items, $first_column_count);
                    ?>
                    
                    <div class="grid md:grid-cols-2 gap-6 pt-4">
                        <!-- First Column -->
                        <div class="space-y-5">
                            <?php foreach ($first_column_items as $item) : 
                        ...
                        <!-- Second Column -->
                        <div class="space-y-5">
                            <?php foreach ($second_column_items as $item) : 
                                    $is_active = $item['active'] ?? false;
                                    $active_class = $is_active ? 'text-[#CB122D]' : 'text-gray-800';
                                    $body_class = $is_active ? '' : 'hidden';
                                    $icon_text = $is_active ? '−' : '+';
